...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create delegation-checker-64fe
  fly apps create delegation-checker-64fe-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app delegation-checker-64fe
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app delegation-checker-64fe-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'delegation-checker-64fe-staging' does not match app name in config file 'delegation-checker-64fe'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name delegation-checker-64fe-db
  fly postgres attach --app delegation-checker-64fe delegation-checker-64fe-db

  fly postgres create --name delegation-checker-64fe-staging-db
  fly postgres attach --app delegation-checker-64fe-staging delegation-checker-64fe-staging-db
  ```
